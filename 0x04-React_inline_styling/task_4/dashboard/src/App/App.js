...
class App extends Component {
  ...
  render() {
    return (
      <Fragment>
        {<div className={this.displayDrawer ? css(styles['mobileNotification']) : ''} style={{ position: 'absolute', right: 0, background: 'white', }}>
          <Notifications displayDrawer={this.displayDrawer} listNotifications={this.state.listNotifications} />
        </div>}
        {<div className={css(styles.app)}
          style={{
            [`@media (maxWidth: ${CSSVariables.mobileMaxWidth})`]: {
              display: this.displayDrawer ? 'none' : 'block'
            }
          }}
        >
          <Header />
